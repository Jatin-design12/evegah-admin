...
const IsPointExistsInPolygon = async (api_polygon_db: any, api_polygon_multiarray: any, latitude: any, longitude: any) => {
    ...
    for (let i = 0; i < polyArray.length; i++) {
        ...
        if ((polyArray[i][1] < polyArray[j][1] && y >= polyArray[i][1] && y <= polyArray[j][1]) || (polyArray[i][1] > polyArray[j][1] && y >= polyArray[j][1] && y <= polyArray[i][1])) {
            // Check if Px (x-componet of our point P) crosses our path
            let sx = polyArray[i][0] + ((polyArray[j][0] - polyArray[i][0]) * (y - polyArray[i][1])) / (polyArray[j][1] - polyArray[i][1]);
            if (sx >= x) intersect += 1;
        }
    }
    if (intersect % 2 === 0) {
