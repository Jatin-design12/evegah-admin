...
function getLocalAssetBaseUrl() {
    ...
    const explicitImageBase = String(process.env.IMAGE_BASE_URL || '')
        .trim()
        .replace(/\/$/, '');
    ...
    const explicitServerBase = String(process.env.SERVER_BASE_URL || '')
        .trim()
        .replace(/\/$/, '');
...
async function setBeepOnInstructionCommon(requstData: any, res: any, req: any) {
    ...
    ((requestBody.beepInstructionEnumIdLog = requestBody.beepInstructionEnumId), (requestBody.geofence_inout_enum_id = 63));
...
async function setBeepOffInstructionCommon(requstData: any, res: any, req: any) {
    ...
    ((requestBody.beepInstructionEnumId = '57'), // beep off
        (requestBody.beepStatusEnumId = null));
    ((requestBody.beepInstructionEnumIdLog = requestBody.beepInstructionEnumId), (requestBody.geofence_inout_enum_id = 62));
...
async function updateApiResponce(requestResponseId: any, responseData: any, responseStatusEnumId: any, exceptionFull: any, exceptionName: any, exceptionMessage: any) {
    try {
        let request_data: any = JSON.stringify(responseData);

        let result = await InwardServices.updateApiResponceDataService(requestResponseId, request_data, responseStatusEnumId, exceptionFull, exceptionName, exceptionMessage);
        if (result) {
...
export {
    ...
    getOriginalFileName,
    checkFileExist,
    covertJsonArrayToDBarray,
    generatedQrCodeForVehicles,
    createAuthToken,
    verifyToken,
    calculateMin,
    encrypt,
    decrypt,
    getTokenDetail,
    fromDateTodateValidations,
    generateUniqueNumber,
    generateOrderNumber,
    setBeepOnInstructionCommon,
    setBeepOffInstructionCommon,
    checkGeoInOout,
    getAddressFromLatLong,
    checkVaidLatLong,
    insertApiRequest,
    insertApiResponceData,
    getApiUrl,
    updateApiResponce,
    insertApiExceptionData,
    calculateSecond,
    isObjectEmpty,
    calculateHr
};
