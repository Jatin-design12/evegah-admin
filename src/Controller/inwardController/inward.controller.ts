...
async function addDeviceDataUDIMAndDeviceFrom(dataDetail: any) {
    try {
        ...
        ((data.internal_batt_v = '0'), (data.device_lock_and_unlock_status = '2')); // unlock,
        ...
        ((data.createdon_date = actionOnDate), (data.remarks = 'Device added from third party server.'));
...
async function ThingsupAPISetLockUnlockApiCalling(deveieData: any, res: Response) {
    ...
    return new Promise((resolve, reject) => {
        axios
            .post(config.THINGS_URL + 'api/button/command/execute', data, {
                headers: {
