...
const insertFarePlanDetailDetailController = async (req: Request, res: Response) => {
   ...
   for( let i =0 ; i <= requestBody.length -1; i++)
   {
      ...
      for( let j =0 ; j <= requestBody.length -1 ; j++)
      {
        ...
        if ( requestBody[i].aplicableDate==requestBody[j].aplicableDate 
...
//-----------------------------------
 ...
 //console.log('check zero requestBody.perMinuteRateMonday ',requestBody.perMinuteRateMonday)
    ...
    //     }
        ...
        if ((requestBody.perMinuteRateMonday < 0 || requestBody.perMinuteRateMonday ==null) && 
...
// area cityId and city match or not 
    ...
    let exitResult : any  = await AreaMasters.getFarePlanExit(requestBody.farePlanId,requestBody.mapCityId,requestBody.areaTypeEnumId,requestBody.areaId,requestBody.modelId,requestBody.aplicableDate, req);   
...
const getReportBikeData = async (req: Request, res: Response) => {
    try {
        ...
        result = await AreaMasters.getReportBikeData(requestQuery.userId,requestQuery.rideBookingId,requestQuery.ridestatus,requestQuery.RequestrideFromDate,requestQuery.RequestrideToDate,requestQuery.bikeId,requestQuery.rideEndByUser, req);
...
const getzoneDetailWithBikeCountList = async (req: Request, res: Response) => {
    try {
        ...
        getOutSideGeoFanceBikeListResult = await adminDashboardServices.getOutSideGeoFanceBikeListZoneWise(requestQuery, req);
...
export default {addUpdateAreaDetailController,getAreaDetail,insertFarePlanDetailDetailController,getfarePlanDetail,getReportBikeData,
    addCountryStateCityController ,
    getMapAreaDetail,getzoneDetailWithBikeCountList ,addUpdateMapCityDetailController,getCityDataForTable,
    getMapCityDetail,getMapCityDetailsForSearche,getzoneDetailWithAllTypeBikeCountList,getAreaMapCityState,
    getMapCityDetailForReport,getMapAreaDetailForReport,getZoneDetailForReportController}
