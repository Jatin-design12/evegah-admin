        ...
        instance.orders.create(options, async(err: Error, order: any) => {
            ...
            paymentDetails.order_response_json = razorpay_response_fromOrderApi;
                 await payment.insertPaymentTransactionByOrder(paymentDetails);
                        return RequestResponse.forbidden(res, err.message, status.exception, err);
            ...
            if (paymentResult.rowCount > 0)
                ...
                return RequestResponse.success(res, 'success', status.success, order);                                
            }
        });
        ...
        if (requestBody.paymentByAdmin == 'PaymentByApp')
            ...
            //    }
                ...
                if(  Number(requestBody.receivedAmount) < Number(getLastMinRecharge))
                    {
                        return RequestResponse.validationError(res, 'You can not recharge with ' + requestBody.receivedAmount+ ' rupee ' + 'you have to recharge with minimum'+ getLastMinRecharge, status.success, []);
                    }
   ...
   let checkCommit:any =false;
    ...
    { 
     ...
     if( Number(minimumDepositAmountEnmTbl) !=  (remainingDepositAmount) ) 
         {
            return RequestResponse.validationError(res, 'Your current deposit amount is ' +  currentDepositAmount +  ' You can not deposit with ' + (requestBody.receivedAmount)  + ' rupee ' + 'you have to deposit with '+ ( Number(minimumDepositAmountEnmTbl) - Number(currentDepositAmount)) + ' rupee.', status.success, []);
         }
...
//   console.log('check body requestBody req',req)
   ...
   let walletAmount:any = await RideBooking.getWalletAmountByUserId(requestBody);
    ...
    if(  Number(requestBody.receivedRechargeAmount) < Number(getLastMinRecharge))
        {
            return RequestResponse.validationError(res, 'You can not recharge with ' + requestBody.receivedRechargeAmount+ ' rupee ' + 'you have to recharge with minimum '+ getLastMinRecharge, status.success, []);
        }
